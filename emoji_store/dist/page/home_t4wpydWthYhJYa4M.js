...
exports.default = {
    ...
    components: { progress: _progress2.default, emojiList: _emojiList2.default, orientation: _orientation2.default, ball: _ball2.default, banList: _banList2.default },
    data: {},
    methods: {},
    computed: {
...
exports.default = {
  ...
  methods: {
    bind: function bind(n) {
      ...
      this.movingIndex = n;
      var ele = getEl(this.$refs['box' + n][0]);
      var opacity_x_origin = '1-abs(x)/600',
          y_origin = 'y+' + this.xArr[n].y;
      _weexBindingx2.default.bind({
        anchor: ele,
        eventType: 'pan',
        props: [{
        ...
        console.log(e.deltaY);
        if (e.state === 'end') {
          // this.bindingxTime(ele,e.deltaY,n);
          _this.xArr[n].y += e.deltaY;
          _this.movingIndex = null;
          // this.xArr[n].y = Math.min(this.xArr[n].y,this.xArr[n].max);
          // this.xArr[n].y = Math.max(this.xArr[n].y,this.xArr[n].min);
        }
      });
    },
    bindingxTime: function bindingxTime(ele, offsetX, n) {
      var self = this;
      var lastY = self.xArr[n].lastY;
      console.log(self.xArr[n].lastY);
      console.log('offsetX' + offsetX);
      var index = Math.floor(offsetX / 60);
      // index = Math.max(0,index);
      index = Math.min(index, 4);
      var changeValue = index * 60;
      var goalY = 50; // 目标点
      var result = _weexBindingx2.default.bind({
        eventType: 'timing',
        props: [{
          element: ele,
          property: 'transform.translateY',
          expression: 'linear(t,' + self.xArr[n].lastY + ',' + goalY + ',500)'
        }]
      }, function (e) {
        //TODO 	
        self.xArr[n].lastY = goalY;
      });

      console.log(self.xArr[n].y);
    },
    ontouchstart: function ontouchstart(n) {
      // this.bind(num);
      // this.move(this.$refs['box'+num][0]);
      // console.log(e);
      console.log(arguments[0]);
      this.startY = arguments[1][0].changedTouches[0].pageY;
      console.log();
    ...
    ontouchmove: function ontouchmove(n) {
      var _y = arguments[1][0].changedTouches[0].pageY;
      var changeY = arguments[1][0].changedTouches[0].pageY - this.startY;
      this.move(this.movingEle);
    },
    move: function move(ele) {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('div', {
      ...
      refInFor: true,
      staticClass: ["box"],
      class: ['box' + index, _vm.movingIndex == index ? 'movingBox' : ''],
      on: {
        "touchstart": function($event) {
