...
var _lastSwapList = __webpack_require__(27);
...
exports.default = {
    ...
    components: { progress: _progress2.default, emojiList: _emojiList2.default, orientation: _orientation2.default, ball: _ball2.default, banList: _banList2.default, lsatSwapList: _lastSwapList2.default },
    data: {},
    methods: {},
    computed: {
...
exports.default = {
  ...
  methods: {
    bind: function bind(n) {
      ...
      var ele = getEl(this.$refs['cell' + n][0]);
      var opacity_x_origin = '1-abs(x)/600',
          y_origin = 'y+' + this.xArr[n];
      _weexBindingx2.default.bind({
        ...
        if (e.state === 'end') {
          _this.opacity = 1 - Math.abs(e.deltaX) / 600;
          _this.xArr[n] += e.deltaY;
        }
      });
    },
  ...
  mounted: function mounted() {
    for (var i = 0, length = this.emojiList.length; i < length; i++) {
      this.xArr.push(0);
    }
  }
};
...
exports.default = {
  created: function created() {
    for (var i = 0, length = this.cellList.length; i < length; i++) {
      var obj = {
        y: 0, // translateY的值
        min: null, // y轴上的最小偏移值
        max: null, // y轴上的最大偏移值. 必须统一一个绝对值
        top: _universalEnv.isWeex ? i * 60 : i * 60 * 2,
        height: _universalEnv.isWeex ? 50 : 100, // list的高度
        index: i, // list的索引
        nowIndex: i // list的当前索引
      };
      var obj1 = this.deepClone(obj);
      this.xArr.push(obj);
      this.newxArr.push(obj1);
    }
  },
  data: function data() {
    return {
      ...
      newxArr: [],
      directionY: true // true往下，false往上
    };
  },
  ...
  methods: {
    ontouchstart: function ontouchstart(n) {
      ...
      this.movingEle = this.$refs['box' + n][0];
      this.movingIndex = n;
      this.curItem = this.cellList[this.movingIndex];
      this.dragBoxTop = this._getTopValueYById(this.startY);
      console.log(this.startY);
    },
    ontouchmove: function ontouchmove(n) {
      var e = arguments[1][0];
      var _y = e.changedTouches[0].pageY;
      this.changeY = _y - this.startY;
      // 需要做距离检测
      this.directionY = this.changeY >= 0 ? true : false;
      this.move(this.$refs.dragBox, this.changeY);
    },
    ontouchend: function ontouchend(n) {
      var e = arguments[1][0];
      this.endY = e.changedTouches[0].pageY;
      console.log('endY:' + this.endY);

      this.goalListIndex = this.getGoalListIndex();
      this.swapList();
      this.movingIndex = null;
      this.goalListIndex = null;
      this.movingEle = null;
    },
    ...
    changList: function changList() {
      var newList = ['item2', 'item1', 'item3', 'item4'];
      this.cellList = newList;
    },
    getGoalListIndex: function getGoalListIndex(offsetY) {
      // 获取目标list索引
      // 比较ofsetY与各个list的top值，如果offsetY的值大于等于某个list的top-list的高度的一半，返回这个list的索引
      var index = this.movingIndex;
      var height = this.$refs['box' + 0][0].getBoundingClientRect().height;
      var arr = this.newxArr;
      if (Math.abs(this.changeY) < height) return index;
      for (var i = 0, len = arr.length; i < len; i++) {
        if (this.endY > arr[i].top && this.endY < arr[i].top + height + 10) {
          index = i.nowIndex;
          console.log('i:' + i);
          break;
        }
    ...
    swapList: function swapList() {
      // 交换list的位置
      // let t = this.xArr[this.movingIndex] ;
      // this.xArr[this.movingIndex] =  this.xArr[this.goalListIndex];
      // this.xArr[this.goalListIndex] = t;
      // 数组赋值
      var t = this.newxArr[this.movingIndex];
      this.newxArr[this.movingIndex] = this.newxArr[this.goalListIndex];
      this.newxArr[this.goalListIndex] = t;
      this.newxArr.map(function (element, i) {
        element.top = _universalEnv.isWeex ? i * 60 : i * 60 * 2;
      });

      var temp = this.xArr[this.movingIndex].top;
      this.xArr[this.movingIndex].top = this.xArr[this.goalListIndex].top;
      this.xArr[this.goalListIndex].top = temp;
      this.xArr[this.movingIndex].nowIndex = this.goalListIndex;
      this.xArr[this.goalListIndex].nowIndex = this.movingIndex;

      // console.log(this.movingIndex+','+this.goalListIndex);
      // console.log(this.xArr[this.movingIndex].nowIndex);
      // console.log(this.xArr[this.movingIndex]);
      // console.log(this.xArr);
    },
    compareUp: function compareUp(propertyName) {
      // 升序排序  
    ...
    return _c('div', {
      ...
      on: {
        ...
        "touchend": function($event) {
          _vm.ontouchend(index, arguments)
        }
      }
    }, [_c('text', {
      staticClass: ["text"]
    }, [_vm._v(_vm._s(item))])])
  }), (_vm.movingIndex !== null) ? _c('div', {
    ref: "dragBox",
...
exports.default = {
  ...
  methods: {
    ontouchstart: function ontouchstart() {
      var e = arguments[1][0];
      this.startY = e.changedTouches[0].pageY;
      this.preY = e.changedTouches[0].pageY - this.$refs.parent.getBoundingClientRect().top;
    ...
    ontouchend: function ontouchend(n) {
      var e = arguments[1][0];
      this.endY = e.changedTouches[0].pageY;
      var collideIndex = this._getIdByPosition(this.endY - this.$refs.parent.getBoundingClientRect().top);
    ...
    swapList: function swapList() {// 交换list的位置
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('div', {
      ...
      class: [_vm.movingIndex == index ? 'movingBox' : ''],
      style: {
        top: _vm.isWeb ? _vm.order[index] * 50 * 2 + 'px' : _vm.order[index] * 60
      },
      on: {
        "touchstart": function($event) {
          _vm.ontouchstart(index, arguments)
