...
<script>
    ...
    methods:{
      buyAction(obj,n) {
          if(this.priceData.price=='') return;
          var type = obj.course_type,
              url  = obj.goto_url+'&htdisablemenu'+'&source='+this.source+'&hpid='+this.hpUserinfo.hpUserId;
          if(type&&/ios/gi.test(WXEnvironment.osName)){ // Ios 未购买提示弹框
              weex.requireModule('modal').alert({
                  message: util.translate('IOS_WARNING'),
                  duration: 100
              })
              return;
          }
          if(type&&!this.isBuy) {           // 收费课程且未购买
            this.$emit('alertBuy',obj,n);
            return;
          }
          if(!type){     // 推荐列表的免费课程
